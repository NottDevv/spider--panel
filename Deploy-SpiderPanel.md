...
| `SECRET_KEY` | `Kx7vQ9mLz2Wp8Rt5Yb3Nf6Hd1Jc4Ga0E` | ✅ | امضای کوکی سشن؛ با مقدار ثابت، بعد از ری‌استارت همه لاگ‌اوت نمی‌شوند |
| `RAILWAY_PUBLIC_DOMAIN` | `a55254-9735.d.jrnm.app` | ✅ | دامنه‌ی عمومی شما؛ برای ساخت کانفیگ‌های WS/XHTTP |
...
> [!IMPORTANT]
> **`RAILWAY_PUBLIC_DOMAIN` حیاتی‌ترین متغیر است!** این مقدار برای ساخت کانفیگ‌های VLESS WS/XHTTP استفاده می‌شود. اگر اشتباه باشد، کانفیگ‌ها وصل نمی‌شوند. مقدار دقیق را از بخش **Public access** پلتفرم بردارید.
...
VLESS Reality یکی از **امن‌ترین و سخت‌ترین** پروتکل‌ها برای شناسایی است. ترافیک شما شبیه ترافیک عادی HTTPS به سایت‌های معتبر (مثل Apple CDN) به نظر می‌رسد.
...
| **EXTERNAL PORT** | `52365` | پورت عمومی از Public access |
...
> [!IMPORTANT]
> **SNI را تغییر ندهید!** پنل از سایت‌های معتبر مثل Apple CDN (`is1-ssl.mzstatic.com`) استفاده می‌کند که بهترین عملکرد را دارند. اگر SNI را تغییر دهید و سایت اشتباه انتخاب شود، کانفیگ وصل نمی‌شود.
...
1. **`RAILWAY_PUBLIC_DOMAIN` درست تنظیم شده باشد** (گام ۳)
...
- ✅ Reality از سایت‌های معتبر برای SNI استفاده می‌کند که شناسایی را سخت می‌کند
...
| علامت | علت | درمان |
|---|---|---|
| **Bad Gateway** بعد از بیلد | ناهماهنگی پورت اپ و پلتفرم | `PORT=80` + پورت پلتفرم `80` + بررسی لاگ‌ها |
...
| `ModuleNotFoundError` | ZIP با پوشه تودرتو | فایل‌ها در ریشه‌ی ZIP باشند |
...
# ── شبکه ────────────────────────────────────
PORT=80
RAILWAY_PUBLIC_DOMAIN=a55254-9735.d.jrnm.app
PYTHONUNBUFFERED=1
